...
class _SaloonDetailsView extends State<SaloonDetailsView> {
  ...
  getInitializeObjects() async {
    final resEmployees =
        await widget.controller.fetchEmployeesList(id: widget.saloon.id!);

    final resServices =
        await widget.controller.fetchServicesList(id: widget.saloon.id!);
    setState(() {
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            ...
            for (var i = 0;
                i < Get.put(SaloonsController()).services.length;
                i++)
              serviceItem(cardHeight, context,
                  service: Get.put(SaloonsController()).services[i]),
